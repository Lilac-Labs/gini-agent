...
- `GET /api/connectors` carries a computed, **non-persisted** `usable` boolean per record. For `google-oauth-desktop` rows it additionally requires at least one signed-in account in the machine-global Google account registry (sign-in liveness folded in at read time — see "Health vs. session liveness").
...
Two read-time surfaces fold liveness in without touching `health`: the transient `usable` bit on `GET /api/connectors` requires at least one signed-in registry account for `google-oauth-desktop` rows, and the `list_connectors` tool returns a top-level `googleAccounts` array (tag, email, `signedIn`, `tokenRevoked`, primary, message) built from the machine-global account registry — present whenever the registry is non-empty, whether or not a google connector record exists. The system-prompt Google accounts block asserts **registration only**: it carries no sign-in state and instructs the model to check `list_connectors` before asserting any account's sign-in status.
