...
const ROUTINES_CONTENT_RULES = [
  ...
  "When supported by the evidence, favor useful patterns such as category-triggered triage and draft replies for a narrow repeated class of inbound; weekly stakeholder or project update drafts assembled from ongoing work; periodic spend, invoice, usage, or operations recaps across recurring vendors; and monitoring credits, benefits, renewals, or expirations so the user can act before value is lost.",
  "These are abstract archetypes, not templates. Choose only the patterns supported by this mailbox and personalize their trigger and output; do not force every archetype or copy details from the examples. Narrow category-specific drafting can qualify, but generic inbox triage does not.",
