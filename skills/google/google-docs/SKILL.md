...
- If `gws auth status` reports no authenticated user, or a Docs call fails with an auth error, call `request_google_account`, tell the user to use the connect/reconnect button, then stop and wait. For `scope required`, name the missing scope before offering the reconnect button.
- OAuth scopes the user picked at login must cover the verbs the agent will use:
  ...
  - The Docs API also accepts the broader `drive` scope, which covers finding docs by title too — an account with a full `drive` grant needs no separate `docs` scope
