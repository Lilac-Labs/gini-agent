...
- Only real people: never the user themselves, never bots or automated senders — including AI assistant/product personas that sign with a human-sounding name (e.g. an assistant that sends "Morning Briefing" emails) and support/service reps acting for a product. Mail from a shared alias (hello@, support@, info@, a store or desk address) is the product's service desk, not a person: a first name signed under one ("Jonathan from support") stays out unless that named human sustains a personal exchange with the user across threads (repeated correspondence, a meeting, a direct address of their own).
